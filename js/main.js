...
(() => {
  ...
  const updateScroll = () => {
    ...
    if (!reduced && scrollY < innerHeight * 1.15) {
      hero.style.setProperty('--hero-scroll', String(Math.min(1, scrollY / innerHeight)));
      const heroVisual = document.querySelector('.hero-visual-depth');
      if (heroVisual) heroVisual.style.translate = `0 ${scrollY * .08}px`;
      document.querySelector('.hero-copy').style.translate = `0 ${scrollY * .06}px`;
    }
  };
  updateScroll();
...
(() => {
  ...
  document.querySelectorAll('.project-screen img').forEach(preview => {
    preview.tabIndex = 0;
